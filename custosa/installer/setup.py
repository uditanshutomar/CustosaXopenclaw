...
def _install_openclaw_guard_plugin(config: CustosaConfig, config_path: Optional[Path]) -> bool:
    ...
    try:
        ...
        # Also copy manifest as clawdbot.plugin.json (OpenClaw checks both names)
        manifest_data = (plugin_src / "openclaw.plugin.json").read_bytes()
        (plugin_dir / "clawdbot.plugin.json").write_bytes(manifest_data)
